...
\begin{proof}
    ...
    $\coprod_{k\geq 0}(\mathcal{D}\mathsf{isk}_{n/M}^{=k})^0$.\marginnote{Change the underlying
    groupoid notation.}
...
\begin{equation*}
    ...
    \end{tikzcd}
\end{equation*}
\begin{theorem}[Smale, after Morse]
...
\marginnote{Learn the 2-sided bar construction.}
...
\begin{theorem}[Convergence of the Eilenberg-Moore spectral sequence]
...
%\begin{equation*}
%    \text{Tor}^{H^*Y}(H^*Y',H^*X) \implies H^X'
%\end{equation*}
...
such that
%\begin{equation*}
%    \text{Tor}^{H^*B}(H^*B',H*A)\implies H^*A'.
%\end{equation*}
\marginnote{Fix these two Tor's}
...
functor $(\int_{D^n}-)^\vee:\mathsf{Alg}_n^\text{op}\to\mathsf{Alg}_n$ (abbreviating
our notation of $n$-disk algebas and omitting the augmentated that is implicit everywhere).
...
given $J\xrightarrow{A}\mathsf{Alg}\xrightarrow{\mathsf{fgt}}\mathsf{Ch}$ one
shows that the limit of this composite functor has an algebra structure. Have
\begin{equation*}
    \begin{tikzcd}
        \lim f\circ A\otimes \lim f\circ A \ar[rr, dashed] \ar[dr] && \lim f\circ A\\
        & \lim (J\times J\xrightarrow{A\otimes A}\mathsf{Ch})\ar[ur]
    \end{tikzcd}
\end{equation*}
...
%Here the vertical map on the left is the monoidal forgetful
%functor $\mathcal{D}\mathsf{isk}_{n-1}\xrightarrow{-\times\R}\mathcal{D}\mathsf{isk}_n$.
