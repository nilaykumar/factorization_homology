...
There are various definitions of $\infty$-categories, and they have their own advantages
and disadvantages. To back up for a moment, suppose we have a category $\mathcal{T}$ with
products, and we have a functor $\Delta\to\mathcal{T}$, then $\Maps_{\mathcal{T}}(s,t)$
is a simplicial set whose $p$-simplices is $\Hom_\mathcal{T}(s\times [p],t)$.
This also, by geometric realization, gives us a space of mappings. If for instance we
take $\mathcal{T}$ to be $\mathsf{Top}$, a topological category, the functor sends
$p\mapsto\Delta^p$. Then $\Maps_\mathsf{Top}(X,Y)_\bullet=\text{Sing}(\Maps(X,Y))$
(recall the adjunction $X\times\Delta^p\to Y \implies \Delta^p\to \Maps(X,Y)$) % TODO two sided arrow

This allows us to give many categories a topological enrichment. For example consider
the functor $\Delta\to \mathsf{CAlg}_\R^\text{op}$ which sends $p\mapsto\Omega^*(\Delta^p)$.
\begin{quote}
...
\end{quote}
The first few examples are of course clear: spaces and simplicial sets. But then
there are the notions of chain complexes and natural transformations of functors.
...
\begin{definition}
    ...
    $\Maps_\mathcal{C} \simeq \Maps_\mathcal{D}(Fc,Fc')$ and every object $d\in\mathcal{D}$
    is homotopy equivalent to some $F(c)$ for $c\in\mathcal{C}$, i.e. there exists a map
    $d\to Fc$ such that $\Maps_\mathcal{D}(e, d)\to \Maps(e,Fc)$ is a homotopy equivalence
    for all $e\in\mathcal{D}$.
\end{definition}

Whatever $\infty$-categories are, they are supposed to have two properties:
\begin{enumerate}
...
When it comes to defining $\infty$-categories, there is a more economical option
and a more conceptual option: quasicategories and complete Segal spaces, respectively.
Let me tell you very briefly about complete Segal spaces (though the main reference is framed
in terms of quasicategories).

When $\mathcal{C}$ is a category, there is a set of objects and a set of morphisms.
However, the only way we ever use categories is up to equivalence, and these sets have
no invariance properties with respect to equivalence. So how can we think of a category
in a way that better reflects this homotopy theory of categories. Well there is a space
of objects of $\mathcal{C}$. Consider the underlying groupoid $\mathcal{C}^0$ where
we have thrown out the noninvertible maps. We can then construct the nerve/classifying space
of $\mathcal{C}^0$. There is also a space of morphisms. The morphisms of $\mathcal{C}$
sits inside of $\Fun^\text{iso}([1], \mathcal{C})$ --- here objects are functors and morphisms
are natural transformations by isomorphisms. This is another groupoid and we can take
its nerve and regard that as the space of morphisms.

Observe now that if $\mathcal{C}\simeq \mathcal{C}'$ then the spaces of objects and morphisms
are equivalent. This construction is a functor $C_\bullet:\mathsf{Cat}\to \Fun(\Delta^\text{op},\mathsf{Spaces})$
sending a category $\mathcal{C}$ to the simplicial space that sends $[p]\mapsto B\left( \Fun^\text{iso}([p],\mathcal{C}) \right)$.
This functor is a fully faithful embedding. It moreover has the property that
...
\begin{equation*}
    ...
    \end{tikzcd}
\end{equation*}
forming a homotopy pullback. This more or less characterizes the image of this functor.
Hence one should think that colimits/limits will be mapped to homotopy colimits/limits, etc.
However, this gets somewhat complicated as we quickly run into bisimplicial sets.

This leads us to the economical option of quasicategories which only deal with
simplicial sets (also there is thousands of pages of reference on it).
\begin{definition}
    A \textbf{quasicategory} $\mathcal{C}$ is a simplicial set such that every
    inner horn (for $n\geq 2$) has a filler.
\end{definition}
Let's explain these terms. Write $\Delta[n]$ for the geometric $n$-simplex (i.e. the
functor $\Delta^\text{op}\to \mathsf{Set}$ given $\Delta[n]=\Hom(-,[n])$).
There are a number of maps $\Delta[n-1]\to \Delta[n]$ given by skipping $i$. We will
define the inner horn $\Lambda_i[n]=\cup\Delta[n-1]$ where the union is over all
those except the $i$th spot. For instance there are three horns of the 2-simplex.
An inner horn is a horn where the missing $n-1$ piece is not the $0$th or $n$th.
Now what does it mean to have a inner filler (for $n\geq 2$)? It means that for
every inner horn $\Lambda_i[n]$ mapping into $\mathcal{C}$,
\begin{equation*}
    \begin{tikzcd}
        \Lambda_i[n] \ar[r]\dar & \mathcal{C} \\
...
\begin{example}
    \begin{enumerate}
        \item Consider $\mathcal{C}=\text{Sing}(X)$. Observe that there exists a retraction
            of the geometric simplex to the geometric horn. Now solving the lifting
            problem is equivalent to solving the lifting problem on the geometric side
            (by adjunction), using this retraction.
        \item Given a category $C$ consider the nerve $\mathcal{C}=NC$. By the Yoneda lemma
            a map $\Lambda_1[2]\to NC$ is precisely the data of giving a composable pair of
            morphisms in $C$. In particular there is a unique way of filling the
            horn into the simplex by composition.
    \end{enumerate}
\end{example}

What we will do next is give the definitions of colimits, limits, functor categories,
and over/undercategories in quasicategories. We will also discuss some variant of
the nerve functor $N:\mathsf{TopCat}\to\mathsf{QuasiCat}$, which in good cases will
send $\hocolim\mapsto\colim$.

Question from Greisha: is there some statement that complete Segal spaces are better?
John: I don't know if this is getting at your question but here's an example.
There is an inclusion $\mathsf{Cat}_\infty\subset\Fun(\Delta^\text{op},\mathsf{spaces})$ so it's
easy to give an internal definition. However the same is not true for quasicategories,
there is no such inclusion.
