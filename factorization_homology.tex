...
\section{More on $\infty$-categories [10/11/2017]}

Recall the definition of a colimit. Suppose we have a functor $F:\mathcal{C}\to \mathcal{D}$.
The \textbf{right cone} $C^>$ is defined to be $\mathcal{C}^>=\mathcal{C}\coprod *$ on objects.
The set of morphisms from $x$ to $y$ is the usual set, unless $y=*$, in which case it will
be a single element set, and unless $x=*$, in which case it will be empty.
The \textbf{undercategory} $\mathcal{D}^{/F}$ is the fiber over $F$,
\begin{equation*}
    \begin{tikzcd}
        \mathcal{D}^{F/} \dar\rar & \Fun(\mathcal{C}^>, \mathcal{D})\dar \\
        \{F\} \rar & \Fun(\mathcal{C},\mathcal{D})
    \end{tikzcd}
\end{equation*}
i.e. has objects $d\in\mathcal{D}$ together with a natural transformation
...
\begin{definition}
    ...
    if there exists a functor $\overline F:\mathcal{C}^>\to \mathcal{D}$ 
    with $\overline F(*)\cong d$ and such that the natural restriction
    $\mathcal{D}^{\overline F/} \to \mathcal{D}^{F/}$ is an equivalence.
\end{definition}

If you're a bit confused about why this is a colimit, observe that the category $\mathcal{C}^>$ has a final object. 
For any $\mathcal{C}'$ which has a final object with a functor $G:\mathcal{C}'\to \mathcal{D}$
then $\mathcal{D}^{G/}\cong \mathcal{D}^{G(*)/}$.
