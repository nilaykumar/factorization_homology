...
$A(\varnothing)=1_{\mathcal{V}}\to A(*)$. Here $1$ is the symmetric monoidal unit in $\mathcal{V}$.
The multiplicative structure comes from the $\Sigma_2$ action on $\{0,1\}\to *$. In
particular, $A^{\otimes\{0,1\}}\to A$, invariant with respect to $\Sigma_2$.
...
Today we will discuss examples of such objects for $\mathcal{V}$ being chain
complexes and toplogical spaces.

Let's start with $\mathcal{V}=\mathsf{Ch}_\Z$.
\begin{enumerate}
    ...
    \item Now let $A:(\mathsf{Fin},\coprod) \to(\mathsf{Ch},\otimes)$ be a commutative dg algebra.
        There is a functor $\pi_0:(\Disk,\coprod)\to (\mathsf{Fin},\coprod)$ which takes
        ...
        For $n$-disk algebras coming commutative algebras, as we've just defined, all of these
        multiplications are the same.

    \item The next example is that of an $n$-fold loop space. We will construct a functor
        $\Disk\to\mathsf{Top}$ and then postcompose with $C_*$ to obtain a chain complex.
        This first functor is $\Omega^nZ:\Disk\to\mathsf{Top}$ for $Z$ a pointed space, which
        we will now define.
        ...
        \end{equation*}
        whose composite we write $C_*\Omega Z$.
    ...
    \item At the opposite end of the spectrum from trivial algebras are free algebras.
        ...
        sends
        \begin{equation*}
            \R^n \mapsto \oplus_{k\geq 0}C_*\left( \Emb^\text{rect}(\coprod_kD^n,D^n) \right)\otimes_{\Sigma_k}V^{\otimes k}.
        \end{equation*}
        ...
        for $A$ an $\mathcal{E}_n$-algebra,
        there exists a unique map of $\mathcal{E}_n$-algebras such that the diagram
        \begin{equation*}
            \begin{tikzcd}
                V \rar\dar & A \\
            ...
            \end{tikzcd}
        \end{equation*}
        commutes. What is this dashed map? For each $k$ we need a map
        \begin{equation*}
            C_*\left( \Emb^\text{rect}(\coprod_k D^n,D^n) \right)\otimes_{\Sigma_k} V^{\otimes k} \to A.
        \end{equation*}
        ...
        and we have $\Emb^{rect}(\coprod_k D^n,D^n)\to \Maps_\mathsf{Ch}(A^{\otimes k}, A)$ which by
        Dold-Kan (recall that everything is enriched in $\mathsf{Top}$) corresponds to a map
        \begin{equation*}
            C_*\left( \Emb^\text{rect}(\coprod_k D^n,D^n) \right)\to \underline{\Hom}(A^{\otimes k},A)
        \end{equation*}
        that is $\Sigma_k$-equivariant. Because of the equivariance it factors to the quotient, which
        gives us the multiplication map. Okay, but we haven't yet shown that the free thing is actually
