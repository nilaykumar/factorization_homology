...
\begin{definition}
    ...
    for any $\kappa$-small $\mathcal{K}$ together with a functor $\mathcal{K}\mathcal{C}$,
...
\begin{definition}
    For $\mathcal{C}$ an ordinary category, we say that $C$ is \textbf{filtered} if for
    all finite $\{x_i\}$ there exists $x$ such that there exists $x_i\to x$ for all $i$
    and for all pairs $f,g:x\to y$, there exists $h:y\to z$ coequalizing $f$ and $g$.
\end{definition}
\begin{example}
    As a simple example consider the category of natural numbers $\N$ with
    unique morphisms $m\to n$ when $m\leq n$.
\end{example}
\begin{example}
...
\begin{example}
    ...
    that this is filtered. For any finite collection $\{U_i\}$ consider $x=\cap_I U_i$,
    which gives us the first condition. Now suppose we have $f, g:U_i\to U_j$. In this
    case we just let $z=U_j$. Hooray.
\end{example}
...
with objects that of $[n]$. As a category, $[n]'\simeq *$. Recall that $\Delta[n]$ is
...
Goette, Igusa, Williams have a theorem (in the psuedoisotopy range) you get all exotic
structures from Hatcher's example. This is related to factorization homology.

\begin{proof}[Proof of HTT 5.3.1.20]
    This proof will be model-dependent. Let $\mathcal{C}$ be a quasicategory. Then
    $B\mathcal{C}=|\mathcal{C}|$. Take any finite $K\hookrightarrow |\mathcal{C}|$
    a subcomplex. This is represented by a map $\bar K\to \mathcal{C}$ where $\bar K$
...
\end{theorem}
\begin{proof}
    We use a hypercover argument. Recall that given $\mathcal{C}\subset \mathsf{Opens}(M)\xrightarrow{\id}\mathsf{Spaces}$
    then $\hocolim_\mathcal{C} \id \simeq M$ if for all $x\in M$ then $B\mathcal{C}_x\simeq *$.
    Apply this fact to $\mathcal{C}={\disk}_{/M}\to \mathsf{Opens}(M)$. We have to show
    that $({\disk}_{/M})_x\simeq *$. We will show that $({\disk}_{/M})_x$ is cofiltered.
    In particular, we need to show (1) for any $U_i\ni x$ then there exists $U$ with $U_i\to U$.
    Take any $U\cong\R^n\subset\cap_i U_i$ containing $x$. Then we need to show (2) that
...
\begin{example}
    Here are a few examples of adjunctions (of $\infty$-categories) to keep in mind.
    \begin{enumerate}
        \item The singular chains $C_*:\mathsf{Spaces}\to \mathsf{Ch}$ has a right
