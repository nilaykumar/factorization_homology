...
Recall that given a functor $A:\mathcal{D}\mathsf{isk}_n^\partial\to \mathcal{V}$, we define
\begin{equation*}
    \int_M A \simeq \colim A.
\end{equation*}
Now there are two fundamental objects of $\mathcal{V}$: $A(\R^n)$ and $A(\R^n_{\geq 0})$.
Now notice that NAPD also works for manifolds with boundary, where
...
\begin{example}
    ...
    homology over the closed $n$-disk deloops!

    Notice, for future reference, that
    \begin{equation*}
...
Matt: what does dualizing mean in $\mathcal{V}$.
John: this will only apply when $\mathcal{V}$ is stable.
