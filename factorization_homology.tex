...
If you're a bit confused about why this is a colimit, observe that the category
...
\begin{definition}
    ...
    we define the mapping space $\Maps_\mathcal{C}(x,y)$ as the fiber
    \begin{equation*}
        \begin{tikzcd}
            \Maps_\mathcal{C} \dar\rar & \Maps(\Delta[1],\mathcal{C})\dar{\ev_0\times\ev_1} \\
...
above:
\begin{align*}
    N(\mathcal{C}^>)_k &= \Fun([k],\mathcal{C}^>) = \coprod_{i\leqslant k}\Fun([i],\mathcal{C})\times \{*\} \sqcup *
    &= \coprod_{i\leqslant k} N(\mathcal{C})_i \times \{*\}\sqcup *.
\end{align*}
This leads us to the following definition.
\begin{definition}
    For $S$ a simplicial set, define the \textbf{right cone} on $S$ to be
    \begin{equation*}
        S^>_k = \coprod_{i\leqslant k}S_i \sqcup *.
    \end{equation*}
\end{definition}
It is easy to see that this naturally forms a simplicial set.

\begin{definition}
    For $F:\mathcal{C}\to \mathcal{D}$ a functor of quasicategories, we define
    the undercategory $\mathcal{D}^{F/}$ to be the fiber
    \begin{equation*}
        \begin{tikzcd}
            \mathcal{D}^{F/} \rar\dar & \Fun(\mathcal{C}^>,\mathcal{D})\dar\\
            \{F\} \rar & \Fun(\mathcal{C},\mathcal{D})
        \end{tikzcd}
    \end{equation*}
\end{definition}
...
\begin{definition}
    We say that $d\in \mathcal{D}$ is a colimit of $F$ if there exists a functor
    $\overline F:\mathcal{C}^>\to \mathcal{D}$ with $F(*)=d$ and
    ...
    \begin{enumerate}
        \item Colimits in a quasicategory are invariant upto categorical equivalence. In other words,
            ...
            \begin{equation*}
                ...
                \end{tikzcd}
            \end{equation*}
            then $\colim_\mathcal{C} F = \colim_{C'}F'.$
...
work through this in the future. But first let's see why these results are nice,
and motivate working with quasicategories.

Consider ${\Diskr}_\infty = \lim_{\to\infty}\Diskr$ the sequential limit.
One finds that ${\Diskr}_\infty\simeq \mathsf{Fin}$. In particular recall that
$\Emb^\text{rect}(\coprod_2 D^n,D^n)\simeq S^{n-1}$ and as $n$ grows large
these approach $S^\infty$, which is contractible. A basic question is
whether there is a factorization
\begin{equation*}
    \begin{tikzcd}
        {\Diskr}_\infty \rar\dar & \mathsf{Top} \\
        \mathsf{Fin}\ar[ur,dashed]
    \end{tikzcd}
\end{equation*}
It turns out that there does not exist such a factorization in general:
\begin{equation*}
    \Fun(\mathsf{Fin},\mathsf{Top}) \not\simeq \Fun({\Diskr}_\infty, \mathsf{Top})
\end{equation*}
This is stemming from the fact that infinite loop spaces are not the same
...
Recall there was to show that there is a continuous assignment $\Maps_c(U,Z)\to\Maps_c(V,Z)$.
Here we give $\Maps_c(U,Z)$ the subspace topology as inherited from $\Maps_*(U^+,Z)$. If we
view it as a subspace of $\Maps(U,Z)$ this is statement is not true. For the next homework
revise this. In addition, do the following exercise, for \textbf{Homework 5}.
...
to prove absolutely everything. The following should be a good pedagogical example
with which we can ``get in and get out'' of the theory of quasicategories.
\begin{proposition}[HTT proposition 1.2.9.3]
    ...
    \end{equation*}
\end{proposition}
Matt asked how this relates to pointwise homotopy invariance. John: this result is a bit harder than that one.
...
\begin{enumerate}
    ...
    \item Given two left fibrations $\mathcal{C}', \mathcal{C}''$ over $\mathcal{C}$, 
        ...
        the map $\mathcal{C}'\times_{\mathcal{C}}\{x\} \to \mathcal{C}''\times_{x}$ is
        an equivalence of Kan complexes.
\end{enumerate}

We'll begin by showing (2). We first need a definition
\begin{definition}
    For $X,Y$ simplicial sets, the join $X\star Y$ is the simplicial set given on
...
nerves of $C$ and $D$. Similarly for spaces.
...
\end{definition}
Notice that ``anodyne'' is an english word meaning roughly inoffensive, bland, unproblematic.
